...
# Entity-type -> semantic_class filters are sourced from the Phase 1.3
# scoping registry so every step uses the same in/out-of-scope decision.
_BIORED_TYPE_TO_CLASS = entity_scope.snomed_normalized_type_map("BioRED")
_CDR_TYPE_TO_CLASS = entity_scope.snomed_normalized_type_map("BC5CDR")
_NCBI_TYPE_TO_CLASS = entity_scope.snomed_normalized_type_map("NCBI_Disease")
_NLM_TYPE_TO_CLASS = entity_scope.snomed_normalized_type_map("NLM-Chem")
...
def harvest_all() -> Dict[str, Dict[Tuple[str, str], int]]:
    ...
    if ncbi_files:
        out["NCBI_Disease"] = harvest_corpus("NCBI_Disease", ncbi_files, _NCBI_TYPE_TO_CLASS)
    else:
        out["NCBI_Disease"] = {}
    ...
    if nlm_files:
        out["NLM-Chem"] = harvest_corpus("NLM-Chem", nlm_files, _NLM_TYPE_TO_CLASS)
    else:
        out["NLM-Chem"] = {}
